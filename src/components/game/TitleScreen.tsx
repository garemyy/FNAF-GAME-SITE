...
export const TitleScreen = ({ onStart }: TitleScreenProps) => {
  ...
  // Creiamo e avviamo subito la musica
  useEffect(() => {
    lobbyMusicRef.current = new Audio("/publica/musiclobby.mp3");
    lobbyMusicRef.current.volume = 0.7;
    lobbyMusicRef.current.loop = true;

    // Avvio immediato della musica
    ...
    return () => {
      ...
      if (lobbyMusicRef.current) lobbyMusicRef.current.currentTime = 0;
    };
  }, []);

  const handleStart = () => {
    // Stop della musica lobby e avvio del Game
